                ...
                <tr className="border-b border-border bg-accent/30">
                  <th className="px-3 py-2 text-left font-medium text-muted-foreground">
                    <button onClick={() => handleSort('title')} className="flex items-center gap-1 hover:text-foreground">
                      {t('todoTitle')} <ArrowUpDown className="h-3 w-3" />
                    </button>
                  </th>
                  <th className="px-3 py-2 text-left font-medium text-muted-foreground">
                  ...
                  <th className="px-3 py-2 text-left font-medium text-muted-foreground">{t('assignee')}</th>
                  <th className="px-3 py-2 text-left font-medium text-muted-foreground">{t('workspace')}</th>
                  <th className="px-3 py-2 text-left font-medium text-muted-foreground">{t('threads')}</th>
                  <th className="px-3 py-2 text-left font-medium text-muted-foreground">
                ...
                {isLoading ? (
                  ...
                  todos.data.map((todo) => (
                    <tr key={todo.id} className="border-b border-border hover:bg-accent/20">
                      ...
                      <td className="px-3 py-2">
                        <span className={`inline-flex items-center rounded-full px-2 py-0.5 text-[10px] font-medium ${statusColors[todo.status] || ''}`}>
                          {t(`status_${todo.status}`)}
                        </span>
                      </td>
                      <td className="px-3 py-2">
                        <span className={`text-xs ${priorityColors[todo.priority] || ''}`}>{todo.priority}</span>
                      </td>
                  ...
                  <tr>
                    <td colSpan={7} className="py-8 text-center text-muted-foreground text-sm">{t('noResults')}</td>
                  </tr>
                )}
              </tbody>
            </table>
        ...
        {todos && todos.totalPages > 1 && (
          <div className="flex items-center justify-between">
            <p className="text-xs text-muted-foreground">
              {t('showing', { from: (page - 1) * limit + 1, to: Math.min(page * limit, todos.total), total: todos.total })}
            </p>
            <div className="flex items-center gap-1">
              <Button variant="outline" size="sm" onClick={() => setPage(Math.max(1, page - 1))} disabled={page <= 1} className="h-7 w-7 p-0">
                <ChevronLeft className="h-3.5 w-3.5" />
              </Button>
              <span className="px-2 text-xs text-muted-foreground">{page} / {todos.totalPages}</span>
              <Button variant="outline" size="sm" onClick={() => setPage(Math.min(todos.totalPages, page + 1))} disabled={page >= todos.totalPages} className="h-7 w-7 p-0">
