            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border bg-accent/30">
                  ...
                  <th className="px-3 py-2 text-left font-medium text-muted-foreground">
                    <button onClick={() => handleSort('slug')} className="flex items-center gap-1 hover:text-foreground">
                      {t('slug')} <ArrowUpDown className="h-3 w-3" />
                    </button>
                  </th>
                  <th className="px-3 py-2 text-left font-medium text-muted-foreground">{t('members')}</th>
                  <th className="px-3 py-2 text-left font-medium text-muted-foreground">{t('todos')}</th>
                  <th className="px-3 py-2 text-left font-medium text-muted-foreground">{t('teamsCount')}</th>
                  <th className="px-3 py-2 text-left font-medium text-muted-foreground">
                ...
                {isLoading ? (
                  ...
                  workspaces.data.map((ws) => (
                    <tr key={ws.id} className="border-b border-border hover:bg-accent/20">
                      ...
                      <td className="px-3 py-2 text-muted-foreground text-xs font-mono">{ws.slug}</td>
                      <td className="px-3 py-2 text-muted-foreground">{ws._count.members}</td>
                      <td className="px-3 py-2 text-muted-foreground">{ws._count.todos}</td>
                      <td className="px-3 py-2 text-muted-foreground">{ws._count.teams}</td>
        ...
        {workspaces && workspaces.totalPages > 1 && (
          <div className="flex items-center justify-between">
            <p className="text-xs text-muted-foreground">
              {t('showing', { from: (page - 1) * limit + 1, to: Math.min(page * limit, workspaces.total), total: workspaces.total })}
            </p>
            <div className="flex items-center gap-1">
              <Button variant="outline" size="sm" onClick={() => setPage(Math.max(1, page - 1))} disabled={page <= 1} className="h-7 w-7 p-0">
                <ChevronLeft className="h-3.5 w-3.5" />
              </Button>
              <span className="px-2 text-xs text-muted-foreground">{page} / {workspaces.totalPages}</span>
              <Button variant="outline" size="sm" onClick={() => setPage(Math.min(workspaces.totalPages, page + 1))} disabled={page >= workspaces.totalPages} className="h-7 w-7 p-0">
